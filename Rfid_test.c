...
int main(void)
{
    ...
    while (1)
    {
        ...
        if (found && is_card_blocked((u8)current_user_index))
        {
            // Card UID matched a known user, but it has been reported lost/stolen -> Access Denied
            CmdLCD(CLEAR_LCD);
            CmdLCD(GOTO_LINE1_POS0);
            StrLCD("Card BLOCKED!");
            CmdLCD(GOTO_LINE2_POS0);
            StrLCD("Report Lost/Stolen");
            CmdLCD(GOTO_LINE3_POS0);
            ...
            delay_ms(2500); // Hold denied indicators active on screen
            
            current_user_index = -1; // Do not open a session for a blocked card
        }
        else if (found)
        {
            CmdLCD(CLEAR_LCD);
        ...
        else if (is_same_id(card, "12528171", 8)) // Officer Card
        {
            CmdLCD(CLEAR_LCD);
            CmdLCD(GOTO_LINE1_POS0);
            StrLCD("Officer Card!");
