...
const char card_ids[NUM_USERS][9] = {
    "12527137",     // Card ID for User 1 (Akash)
    "12536316",     // Card ID for User 2 (Tagore)
    "12632238"      // Card ID for User 3 (Mithul)
};
...
// Card status bytes: 0x00 = ACTIVE, 0x01 = BLOCKED (lost/stolen). Used one byte per user.
const u16 eeprom_block_addrs[NUM_USERS] = { 0x0050, 0x0051, 0x0052 };

// Party names shown on the voting screen (index 0..3 maps to keys '1'..'4')
const char party_names[4][4] = {
    "BJP",
    "INC",
    "AAP",
    "BSP"
...
{
    // NOTE: Magic byte bumped from 0xC5 -> 0xC6.
    // Any board that was previously flashed (and already has 0xC5 stored,
    // possibly with stale/different PIN data at the same addresses) will
    // NOT match 0xC6, so it force-reinitializes ONCE with fresh "1234"
    // PINs for all users below. After that first boot, 0xC6 is stored and
    // it will no longer re-run this block on subsequent power-ups.
    u8 magic = ByteRead_25LC512(0x0000);
    if (magic != 0xC6)
    {
        // Write magic byte
        ByteWrite_25LC512(0x0000, 0xC6);
        delay_ms(5);
        ...
        // Initialize all 3 users' card status as ACTIVE (not blocked)
        ByteWrite_25LC512(eeprom_block_addrs[0], 0x00); delay_ms(5);
        ByteWrite_25LC512(eeprom_block_addrs[1], 0x00); delay_ms(5);
        ByteWrite_25LC512(eeprom_block_addrs[2], 0x00); delay_ms(5);
    }
}

/**
 * @brief  Check whether a user's card is currently BLOCKED (e.g. reported lost/stolen).
 *         Any EEPROM value other than 0x01 is treated as ACTIVE, so a blank/unprogrammed
 *         EEPROM cell (0xFF) is safely treated as ACTIVE on old boards.
 * @return 1 if blocked, 0 if active.
 */
u8 is_card_blocked(u8 idx)
{
    u8 status;
    if (idx >= NUM_USERS) return 0;
    status = ByteRead_25LC512(eeprom_block_addrs[idx]);
    return (status == 0x01) ? 1 : 0;
}

/**
 * @brief  Directly SET a user's card status to BLOCKED or ACTIVE and persist it to
 *         EEPROM (not a toggle). Called from the Officer Menu when a card is
 *         reported lost/stolen (block) or recovered/reissued (unblock).
 * @param  idx     User index (0..NUM_USERS-1)
 * @param  blocked 1 to BLOCK the card, 0 to UNBLOCK (make ACTIVE) the card
 */
void set_block_status(int idx, u8 blocked)
{
    if (idx < 0 || idx >= NUM_USERS) return;
    ...
    if (blocked)
    {
        ByteWrite_25LC512(eeprom_block_addrs[idx], 0x01); // BLOCKED
        delay_ms(5);
        StrLCD("Card BLOCKED:");
    }
    else
    {
        ByteWrite_25LC512(eeprom_block_addrs[idx], 0x00); // ACTIVE
        delay_ms(5);
        StrLCD("Card UNBLOCKED:");
    }
...
/**
 * @brief  Let the officer pick a user by typing their User No. (1..NUM_USERS)
 *         on the keypad. Used for Block/Unblock, since the citizen's card may
 *         be physically lost and cannot be scanned.
 * @return User index (0..NUM_USERS-1), or -1 if cancelled/timed out/invalid.
 */
int select_user_by_number(void)
{
    char key;
    u8 i;
    ...
    CmdLCD(CLEAR_LCD);
    CmdLCD(GOTO_LINE1_POS0);
    StrLCD("Enter User No:");
    // Show one user per line (LINE2..LINE4) so longer names like "Mihir"
    // are never cut off by the 20-column display width.
    for (i = 0; i < NUM_USERS && i < 3; i++)
    {
        if (i == 0) CmdLCD(GOTO_LINE2_POS0);
        else if (i == 1) CmdLCD(GOTO_LINE3_POS0);
        else CmdLCD(GOTO_LINE4_POS0);
        
        CharLCD('1' + i);
        StrLCD(".");
        StrLCD((s8 *)names[i]);
        
        if (i == 2) StrLCD("  *:Cancel"); // Append cancel hint next to the last user
    }
    
    key = KeyScanWithTimeout(20000);
    if (key == 0 || key == '*' || auto_logout_flag) return -1;
    
    if (key >= '1' && key <= '9')
    {
        u8 idx = (u8)(key - '1');
        if (idx < NUM_USERS) return (int)idx;
    }
    
    // Invalid entry
    CmdLCD(CLEAR_LCD);
    CmdLCD(GOTO_LINE1_POS0);
    StrLCD("Invalid User No!");
    delay_ms(1500);
    return -1;
}
...
u8 verify_password_flow(u8 type)
{
    ...
    while (attempts > 0)
    {
        ...
        // Sanity check: EEPROM should always hold ASCII digits '0'-'9' for a
        // valid PIN. If it doesn't (e.g. 0xFF from an unwired/blank chip, or
        // a failed SPI transaction), this is a hardware/EEPROM fault, not an
        // actually-wrong PIN entry -- surface it distinctly so it isn't
        // mistaken for a mistyped PIN.
        {
            u8 j, eeprom_fault = 0;
            for (j = 0; j < 4; j++)
            {
                if (correct_pin[j] < '0' || correct_pin[j] > '9')
                {
                    eeprom_fault = 1;
                    break;
                }
...
void VOTE_menu(void)
{
    ...
    while (1)
    {
        ...
        CmdLCD(CLEAR_LCD);
        CmdLCD(GOTO_LINE1_POS0);
        StrLCD("1."); StrLCD((s8 *)party_names[0]); StrLCD("   2."); StrLCD((s8 *)party_names[1]);
        CmdLCD(GOTO_LINE2_POS0);
        StrLCD("3."); StrLCD((s8 *)party_names[2]); StrLCD("   4."); StrLCD((s8 *)party_names[3]);
        CmdLCD(GOTO_LINE3_POS0);
        StrLCD("5:Exit");
...
/**
 * @brief  Check whether a user's Driving Licence has expired, based on the
 *         current RTC date vs. that user's stored exp_days/exp_months/exp_years.
 *         Shared by the citizen-facing status check and the officer-side
 *         edit-permission gate so both always agree on the same result.
 * @return 1 if expired, 0 if still valid.
 */
u8 is_license_expired(u8 idx)
{
    u8 d = DOM;
    u8 m = MONTH;
    u16 y = YEAR;
    
    if (idx >= NUM_USERS) return 1; // Safety default: treat unknown index as expired
    
    if (y > exp_years[idx]) return 1;
    if (y == exp_years[idx] && m > exp_months[idx]) return 1;
    if (y == exp_years[idx] && m == exp_months[idx] && d > exp_days[idx]) return 1;
...
void show_license_details(void)
{
    ...
    while (1)
    {
        ...
        m = MONTH;
        y = YEAR;
        expired = is_license_expired((u8)current_user_index);
        short_class = "2/4W";
...
/**
 * @brief  Officer-only Driving Licence expiry editor. Lets the officer type the
 *         complete expiry date in one go as 8 digits (DDMMYYYY) which are
 *         auto-formatted on screen as DD/MM/YYYY (e.g. 20/06/2030), then
 *         validates and stores it into exp_days/exp_months/exp_years.
 *         This function is only ever reached from officer_menu() after the
 *         Officer Card has been scanned and verified, so normal citizen
 *         cards never get edit access -- see DRIVING_menu()/show_license_details()
 *         for the citizen-side read-only Valid/Expired check.
 */
void license_edit_menu(int target_idx)
{
    ...
    while (1)
    {
        ...
        while (1)
        {
            if (auto_logout_flag) return;
            key = KeyScanWithTimeout(20000);
            if (key == 0 || key == 0xFE) return; // Timeout/redraw -> exit editor
            ...
            if (key >= '0' && key <= '9')
            {
                if (len < 8)
                {
                    ...
                    len++;
                    CharLCD(key);
                    if (len == 2 || len == 4) CharLCD('/'); // Auto-insert separators after DD and MM
                }
            }
            else if (key == '*') // Backspace, or Exit when already empty
            {
                if (len > 0)
                {
                    len--;
                    ...
                    // Redraw the input line from scratch so the '/' separators stay correct
                    CmdLCD(GOTO_LINE3_POS0);
                    StrLCD("                    "); // Clear 20 columns
                    CmdLCD(GOTO_LINE3_POS0);
                    for (i = 0; i < len; i++)
                    {
                        CharLCD(digits[i]);
                ...
                {
                    return; // Cancel -> back to Driving Lic submenu
                }
            }
            else if (key == '#')
            {
                if (len == 8) break; // All 8 digits entered, go validate
            }
        }
        
        d = (u8)((digits[0] - '0') * 10 + (digits[1] - '0'));
        m = (u8)((digits[2] - '0') * 10 + (digits[3] - '0'));
        y = (u16)((digits[4] - '0') * 1000 + (digits[5] - '0') * 100 + (digits[6] - '0') * 10 + (digits[7] - '0'));
        
        if (d >= 1 && d <= 31 && m >= 1 && m <= 12 && y >= 2000 && y <= 2099)
        {
            exp_days[target_idx] = d;
            exp_months[target_idx] = m;
            exp_years[target_idx] = y;
            
            CmdLCD(CLEAR_LCD);
            CmdLCD(GOTO_LINE1_POS0);
            StrLCD("Expiry Updated!");
            CmdLCD(GOTO_LINE2_POS0);
            if (d < 10) CharLCD('0');
            U32LCD(d); CharLCD('/');
            if (m < 10) CharLCD('0');
...
void officer_menu(void)
{
    ...
    while (1)
    {
        ...
        else if (key == '2')
        {
            ...
            while (1)
            {
                ...
                else if (sub_key == '2')
                {
                    ...
                    if (target != -1)
                    {
                        if (!is_license_expired((u8)target))
                        {
                            // Licence is already valid -- no edit needed, don't open the date editor
                            CmdLCD(CLEAR_LCD);
                            CmdLCD(GOTO_LINE1_POS0);
                            StrLCD("Already Valid,");
                            CmdLCD(GOTO_LINE2_POS0);
                            StrLCD("No Change");
                            CmdLCD(GOTO_LINE3_POS0);
                            StrLCD((s8 *)names[target]);
                            delay_ms(2000);
                        }
                        else
                        {
        ...
        {
            // Block / Unblock a user's card (e.g. lost or stolen card report).
            // Card cannot be scanned since it may be physically lost, so the
            // officer enters the User No. (1..NUM_USERS) on the keypad instead.
            int target = select_user_by_number();
            if (target != -1)
            {
                char action_key;
                ...
                CmdLCD(GOTO_LINE2_POS0);
                StrLCD("Status: ");
                StrLCD(is_card_blocked((u8)target) ? "BLOCKED" : "ACTIVE");
                CmdLCD(GOTO_LINE4_POS0);
                StrLCD("1:Block 2:Unblock");
                
                action_key = KeyScanWithTimeout(20000);
                if (action_key == '1')
                {
                    if (is_card_blocked((u8)target))
                    {
                        // Already blocked -- avoid an unnecessary redundant EEPROM write
                        CmdLCD(CLEAR_LCD);
                        CmdLCD(GOTO_LINE1_POS0);
                        StrLCD("Already Blocked!");
                        CmdLCD(GOTO_LINE2_POS0);
                        StrLCD((s8 *)names[target]);
                        delay_ms(2000);
                    }
                    ...
                    {
                        set_block_status(target, 1); // Block directly
                    }
                }
                else if (action_key == '2')
                {
                    if (!is_card_blocked((u8)target))
                    {
                        // Already active -- do not unblock an already-active card again
                        CmdLCD(CLEAR_LCD);
                        CmdLCD(GOTO_LINE1_POS0);
                        StrLCD("Already Unblocked!");
                        CmdLCD(GOTO_LINE2_POS0);
                    ...
                    {
                        set_block_status(target, 0); // Unblock directly
                    }
                }
                // any other key (including '*' or timeout) simply goes back
            }
            while (ColScan() == 0) delay_ms(10); // Wait for key release before scanning parent menu
        }
    }
